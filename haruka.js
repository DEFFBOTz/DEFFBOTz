...
const starts = async (haruka = new WAConnection()) => {
    ...
    haruka.browserDescription = [ 'DEFFBOTz', 'Chrome', '3.0' ]
	CFonts.say('Haruka', {
		font: 'block',
    	color: ['#ff9c00'],
    	align: 'center',
	...
	haruka.on('group-participants-update', async (anu) => {
		...
		try {
						...
						if (anu.action == 'add' && !num.includes(haruka.user.jid)) {
							...
							try{
							await sendButLoc(mdata.id, `Selamat Datang @${num.split('@')[0]} Di Group ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,welcome, [{"buttonId": `.welcome`,"buttonText": {"displayText": "Welcome 🤗"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Welcome @${num.split('@')[0]} to ${mdata.subject}` + '\n' + lang.welcome(), `Welcome Message By ${ownername}`,ppmem2, [{"buttonId": `.welcome`,"buttonText": {"displayText": "Welcome 🤗"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
							...
							try{
							await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\nSemoga Tenang Di Alam Sana` + '\n' + lang.leave(), `Leave Message By ${ownername}`,goodbye, [{"buttonId": `.out`,"buttonText": {"displayText": "Bye 🤲"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							} catch {
								await sendButLoc(mdata.id, `Goodbye @${num.split('@')[0]}\nSemoga Tenang Di Alam Sana` + '\n' + lang.leave(), `Leave Message By ${ownername}`,ppmem2, [{"buttonId": `.out`,"buttonText": {"displayText": "Bye 🤲"},"type": "RESPONSE"}], {contextInfo: { mentionedJid: [num]}})
							}
